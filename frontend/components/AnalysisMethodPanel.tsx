    ...
    <div className="card">
      ...
      <div className="mt-4 p-4 rounded-lg" style={{ background: "var(--color-navy-700)", border: "1px solid #C9A84C33" }}>
        ...
        <p className="text-xs text-[var(--color-text-muted)] leading-relaxed">
          {lang === "en"
            ? "Each agent outputs a rate path delta (bps) and confidence score, with the market-implied forward path (2Y−DFF × empirical pass-through) injected as a shared anchor. The Chief Orchestrator computes a precision-weighted mean (weight ∝ agent weight × confidence), lifting aggregate confidence ~18% on tight consensus and cutting it ~22% on wide disagreement (capped at 93%). The result passes through a 4-stage stabilizer: (1) EMA smoothing — α=0.25 normal, α=0.60 on FOMC/CPI/NFP days; (2) Half-quantum gate — changes under 12.5 bps are discarded as noise; (3) Conviction gate — low-confidence signals blocked unless it's an event day; (4) 25 bps quantization. The published call and its confidence are the committee's own weighted decision."
            : "각 에이전트는 금리 경로 변화(bps)와 신뢰도를 출력하며, 시장내재 선도 경로(2Y−DFF × 실증 전가율)가 공통 앵커로 주입됩니다. 수석 오케스트레이터가 정밀도 가중 평균(가중치 ∝ 에이전트 가중치 × 신뢰도)을 계산하고, 컨센서스가 좁으면 종합 신뢰도를 약 18% 높이고 분산이 크면 약 22% 낮춥니다(최대 93%). 결과는 4단계 안정화를 거칩니다: (1) EMA 스무딩 — 평시 α=0.25, FOMC/CPI/NFP 당일 α=0.60; (2) 하프-퀀텀 게이트 — 12.5 bps 미만 변화는 노이즈로 폐기; (3) 컨빅션 게이트 — 저신뢰 신호는 이벤트일 외 차단; (4) 25 bps 양자화. 발행되는 콜과 신뢰도는 위원회 자체의 가중 결정입니다."
          }
        </p>
      </div>
    </div>
