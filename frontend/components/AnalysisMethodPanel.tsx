    ...
    <div className="card">
      ...
      <div className="mt-4 p-4 rounded-lg" style={{ background: "var(--color-navy-700)", border: "1px solid #C9A84C33" }}>
        ...
        <p className="text-xs text-[var(--color-text-muted)] leading-relaxed">
          {lang === "en"
            ? "Each agent outputs a rate path delta (bps) and confidence score. The Chief Orchestrator applies a weighted average (total weight varies by agent, event, and limited-mode status). The result passes through a 4-stage stabilizer: (1) EMA smoothing — α=0.25 normal, α=0.60 on FOMC/CPI/NFP days; (2) Half-quantum gate — changes under 12.5 bps are discarded as noise; (3) Conviction gate — low-confidence signals blocked unless it's an event day; (4) 25 bps quantization — published forecast moves only in Fed-move increments."
            : "각 에이전트는 금리 경로 변화(bps)와 신뢰도를 출력합니다. 수석 오케스트레이터가 에이전트·이벤트·제한 모드에 따른 가중 평균을 계산합니다. 결과는 4단계 안정화 파이프라인을 거칩니다: (1) EMA 스무딩 — 평시 α=0.25, FOMC/CPI/NFP 당일 α=0.60; (2) 하프-퀀텀 게이트 — 12.5 bps 미만 변화는 노이즈로 폐기; (3) 컨빅션 게이트 — 저신뢰 신호는 이벤트일 외 차단; (4) 25 bps 양자화 — 게시 예측은 Fed 인상/인하 단위로만 변동."
          }
        </p>
      </div>
    </div>
