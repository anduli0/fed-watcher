...
async def run_prompt(
    ...
    """Run one headless synthesis; return raw stdout text."""
    exe = find_cli()
    if exe is None:
    ...
    if proc.returncode != 0 or "failed to authenticate" in out.lower():
        raise _classify_failure(proc.returncode, out, err)
    if not out:
        raise ClaudeUnavailableError(f"claude -p produced no output: {err[:200]}")
    return out
