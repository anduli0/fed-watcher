...
|---|---|---|---|---|
| `wake-krw-cloud.yml` | 클라우드 | 06:20~07:40 매 10분 핑 | **기상** — Render를 06:40 내부 사이클 전에 깨워 8시 전 완료 보장 | `.github/workflows/` |
| 앱 내부 스케줄러 | 클라우드 | 06:40 사이클 · 07:30 브리핑 | 기동 | Render env `CYCLE_HOURS_KST=6,13,20` |
| `daily-update-krw.yml` | 클라우드 | 07:50 + 09:50/12:50 스윕 | 검증·복구 (그래도 미완료면 POST로 기동) | `.github/workflows/` |
| 앱 내부 스케줄러 | PC 퍼널 | ~07:30 완료 (+ 부팅 warm-up) | 기동 | PC 앱 자체 |
| `daily-update.yml` | PC 퍼널 | 07:50/08:50 + 09:50~14:50 매시 | 검증 (외부 기동은 401 — 미완료 시 앱 자체 완료를 최대 60분 대기) | `.github/workflows/` |
| PC 로컬 작업 | PC 퍼널 | 07:45, 놓치면 부팅 즉시 (`StartWhenAvailable`) | 복구 (localhost POST) | `scheduler/daily_update_krw.ps1` |
| Claude Routine (감시견) | 전부 (krw 클라우드·PC, Fed, KOSPI) | 08:10 — `daily-update-krw/-fed/-kospi/daily-update.yml`의 오늘 성공 검증, 없으면 dispatch, 조치·이상 시에만 푸시 알림 | 감시·보고 | claude.ai/code → Routines → "아침 8시 업데이트 통합 감시견 (08:10 KST, 4개 대상)" |

클라우드 레이어들은 사용자 기기가 전부 꺼져 있어도 동작한다. `wake-krw-cloud.yml`이
Render 무료 티어를 06:40 내부 사이클 전에 깨워두므로, 밤새 잠들어 있었어도
아침 사이클이 제때 돌아 8시 전에 완료된다(waker가 실패해도 07:50 복구가
차선책으로 커버 — 그 경우만 완료가 8시를 살짝 넘길 수 있고 "되는 대로 즉시"
원칙에 따라 곧바로 채워진다). PC 퍼널 사이트가 오프라인이면(PC 꺼짐) 검증
워크플로우는 "SITE OFFLINE"으로 표시하고 오후까지 재검증하며, PC가 켜지는
순간 앱과 로컬 작업이 따라잡는다.

(참고: `scheduler/daily_update.ps1`은 별개인 **Fed-Watcher** 백엔드의
아침 wake-up 폴백, `daily-wakeup.yml`/`daily-update-fed.yml`은 그 클라우드
버전, `daily-update-kospi.yml`/`daily-update-market.yml`은 KOSPI·market
워처용이다.)
