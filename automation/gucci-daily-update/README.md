...
https://anduli0.github.io/gucci-intel-site/ 를 매일 아침 08:00 KST에 자동 업데이트하는 3중 구성.

## 구조

```
[1차] 클라우드 정기 실행   매일 08:00 KST  ← Claude(Max 구독) 클라우드 예약 작업
[2차] 클라우드 보정 실행   매일 10:10 KST  ← 1차 미실행 감지 시에만 대신 실행
[3차] PC 폴백 (선택)       매일 10:30 KST  ← 1·2차 모두 실패 시에만 실제 동작
```
...
- 예약 시각을 놓치거나 실행이 실패하면 2차(10:10 KST)가 자동으로 감지해 즉시 대신 실행한다
  ("8시에 안 되면 되는대로 바로").
...
| 이름 | cron (UTC) | KST | 알림 |
|---|---|---|---|
| Gucci Intel 일간 업데이트 (08:00 KST) | `0 23 * * *` | 08:00 | 푸시+이메일 |
| Gucci Intel 미실행 보정 체크 (10:10 KST) | `10 1 * * *` | 10:10 | 없음(보정 실행 시에만 세션 기록) |
...
동작 방식: 매일 10:30에 라이브 사이트의 `api/summary` 날짜를 확인 → 이미 오늘 날짜면 즉시 종료(중복 실행 없음) → 오래됐으면 로컬에서 `claude -p`로 런북을 실행해 업데이트·푸시. PC가 10:30에 꺼져 있었으면 켜질 때 즉시 실행된다. 로그는 `~\gucci-intel-logs\`에 남는다.
